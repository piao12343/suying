...
v5: 自定义封面优先。点击封面“完成”后等待抖音页面自然接收封面状态；
    ...
    云端浏览器时区固定为 Asia/Shanghai, 避免定时发布被 UTC 偏移 8 小时。
"""
import sys
...
def patch():
    ...
    # ── 步骤 2a: GitHub Actions 浏览器默认 UTC, 抖音定时控件会产生 +8 小时偏移 ──
    tz_old = (
    ...
    if 'timezone_id="Asia/Shanghai"' in code:
        print('[OK] step2a: browser timezone already Asia/Shanghai')
    elif tz_old in code:
        code = code.replace(tz_old, tz_new)
        print('[OK] step2a: browser timezone -> Asia/Shanghai')
    else:
        print('[FAIL] step2a: browser context marker not found')
        ok = False
